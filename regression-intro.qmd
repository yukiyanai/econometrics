...
PDFに render する場合には、`texreg()` を使う（出力は省略する）。
```r
#| results: "asis"
texreg(models, 
       stars = NULL,
...
課題やレポートを提出する際にはQuarto文書をPDFにするので`texreg` を使う。そのとき、[チャンクオプション](https://www.jaysong.net/RBook/quarto1.html#%E3%83%81%E3%83%A3%E3%83%B3%E3%82%AF%E3%82%AA%E3%83%97%E3%82%B7%E3%83%A7%E3%83%B3)に `#| results: "asis"` を付ける（`texreg`を使うチャンクの第1行に `#| results: "asis"`と書く）。
